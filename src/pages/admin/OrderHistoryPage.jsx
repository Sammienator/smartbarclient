            ...
            <tbody>
              {orders.map((o) => (
                <tr key={o.orderId} className="border-t border-ink/5">
                  <td className="py-2 text-ink dark:text-paper">{o.tableNumber}</td>
                  <td className="py-2 text-ink/70 dark:text-paper/70">{o.waiterName}</td>
                  <td className="py-2 text-ink/50 dark:text-paper/50">
                    {(o.items || []).map((it) => `${it.quantity}× ${it.name}`).join(", ")}
                  </td>
                  <td className="py-2 font-mono text-ink/70 dark:text-paper/70">KES {o.totalAmount}</td>
                  <td className="py-2 font-mono text-ink/50 dark:text-paper/50 text-xs">{timeCell(o.placedAt)}</td>
                  <td className="py-2 font-mono text-ink/50 dark:text-paper/50 text-xs">{timeCell(o.kitchenReadyAt)}</td>
                  <td className="py-2 font-mono text-ink/50 dark:text-paper/50 text-xs">{timeCell(o.barReadyAt)}</td>
                  <td className="py-2 font-mono text-ink/50 dark:text-paper/50 text-xs">{timeCell(o.completedAt)}</td>
                  <td className="py-2 text-right">
                    <button
                      onClick={() => setReceiptOrder(o)}
                      className="text-xs rounded-lg bg-white dark:bg-ink-soft border-2 border-ink/20 text-ink/70 dark:text-paper/70 px-2.5 py-1 hover:border-ink dark:border-ink-line dark:hover:border-paper hover:text-ink dark:text-paper dark:hover:text-paper transition-colors"
                    >
                      Print receipt
                    </button>
                  </td>
