    ...
    <motion.div layout initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} className="glow-panel rounded-xl border-2 border-ink/15 dark:border-ink-line bg-white dark:bg-ink-soft p-4">
      <div className="flex items-center justify-between mb-2">
        <p className="font-display font-bold text-ink dark:text-paper">
          ...
          {(order.paymentMethod || order.pin) && (
            <span className="ml-2 text-[10px] font-mono uppercase tracking-wide text-moss bg-moss/15 px-1.5 py-0.5 rounded border border-moss/30">
              Paid{order.paymentMethod === "mpesa" ? " · M-Pesa" : order.paymentMethod === "visa" ? " · Card" : ""}
            </span>
          )}
        </p>
